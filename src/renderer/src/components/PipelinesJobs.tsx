        ...
        <div className="h-full flex flex-col text-white overflow-hidden pb-4">
            ...
            <div className="flex-1 flex gap-6 overflow-hidden">
                ...
                <div className="w-72 flex flex-col gap-4 overflow-hidden">
                    <Card className="flex-1 flex flex-col overflow-hidden bg-black/20 border-outline-subtle">
                        <div className="p-4 border-b border-outline-subtle flex justify-between items-center bg-hover">
                            <h3 className="text-[11px] font-bold text-foreground-muted uppercase tracking-wide">Pipelines</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-3 space-y-2">
                            {filtered.map(p => (
                                <div
                                    ...
                                    className={`group w-full flex items-center justify-between p-3 rounded-xl border text-left cursor-pointer transition-all ${active?.id === p.id
                                        ...
                                        : 'bg-elevated border-outline-subtle hover:border-outline-strong hover:bg-hover'}`}
                                >
                                    <div className="min-w-0">
                                        <div className="text-sm font-bold text-foreground-secondary truncate">{p.name}</div>
                                        <div className="text-[10px] text-foreground-muted uppercase tracking-wide font-bold mt-0.5">
                                    ...
                                    <button
                                        ...
                                        className="p-2 text-foreground-subtle hover:text-red-400 hover:bg-red-400/10 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                                    >
                            ...
                            {pipelines.length === 0 && !loading && (
                                <div className="p-8 text-center border-2 border-dashed border-outline-subtle rounded-2xl">
                                    <p className="text-foreground-muted text-sm">{t('pipelines.noJobs')}</p>
                                </div>
                            )}
                        </div>
                    </Card>
                    ...
                    {/* Add Step Buttons */}
                    {active && (
                        <Card className="p-4 bg-black/40 border-outline-subtle">
                            <h3 className="text-[11px] font-bold text-foreground-muted uppercase tracking-wide mb-3">Add Step</h3>
                                ...
                                {(Object.entries(NODE_TYPE_META) as [NodeType, typeof NODE_TYPE_META[NodeType]][]).map(([type, meta]) => {
                                    ...
                                    return (
                                        <button key={type} type="button" onClick={() => addNode(type)}
                                            className="w-full flex items-center gap-3 p-2.5 bg-elevated border border-outline-subtle rounded-xl hover:bg-hover transition-colors text-left">
                                            <div className="p-1.5 rounded-lg bg-black/40"><Icon className={`w-4 h-4 ${meta.color}`} /></div>
                                            <span className="text-[11px] font-bold text-foreground-secondary">{meta.label}</span>
                                        </button>
                                    )
                                })}
                            </div>
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                    {active ? (
                        <>
                            {/* Pipeline Name */}
                            <Card className="px-5 py-3 bg-black/20 border-outline-subtle flex items-center gap-4">
                            ...
                            <div className="flex-1 overflow-y-auto space-y-2">
                                {nodes.length === 0 ? (
                                    <Card className="p-12 text-center bg-black/20 border-outline-subtle border-2 border-dashed">
                                        <p className="text-foreground-muted text-sm">Add steps from the sidebar to build your pipeline.</p>
                                        <p className="text-foreground-subtle text-xs mt-1">Steps run sequentially — each step's output feeds into the next.</p>
                                    </Card>
                                ) : (
                                    nodes.map((node, idx) => {
                                        ...
                                        return (
                                            <Card key={node.id} className="bg-black/20 border-outline-subtle overflow-hidden">
                                                <div className="flex items-center gap-3 px-4 py-3">
                                                    <div className="flex flex-col gap-0.5">
                                                        <button type="button" onClick={() => moveNode(idx, -1)} disabled={idx === 0}
                                                            className="text-foreground-subtle hover:text-foreground disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-[10px]" aria-label="Move step up">▲</button>
                                                        <button type="button" onClick={() => moveNode(idx, 1)} disabled={idx === nodes.length - 1}
                                                            className="text-foreground-subtle hover:text-foreground disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-[10px]" aria-label="Move step down">▼</button>
                                                    </div>
                                                    <span className="text-[10px] text-foreground-subtle font-bold w-5 text-right">{idx + 1}</span>
                                                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center bg-black/40 border border-outline-subtle`}>
                                                    ...
                                                    <span className="text-[10px] text-foreground-subtle uppercase font-bold tracking-wide">{meta.label}</span>
                                                ...
                                                {isExpanded && (
                                                    <div className="border-t border-outline-subtle px-4 py-3 bg-black/20 space-y-3">
                                                        {node.type === 'llm' && (
                                                            <div className="flex flex-col gap-1">
                                                                <label className="text-[10px] font-bold text-foreground-muted uppercase">System Prompt</label>
                                                                <textarea value={node.data.systemPrompt || ''} onChange={(e) => updateNode(node.id, { systemPrompt: e.target.value })}
                                                                    rows={3} className="bg-black/40 border border-outline rounded px-3 py-2 text-xs text-white outline-none focus:border-blue-500 font-mono resize-y" />
                                                            </div>
                                                        )}
                                                        {node.type === 'tool' && (
                                                            <div className="flex flex-col gap-1">
                                                                <label className="text-[10px] font-bold text-foreground-muted uppercase">Shell Command</label>
                                                                <input value={node.data.command || ''} onChange={(e) => updateNode(node.id, { command: e.target.value })}
                                                                    placeholder="echo $NODE_INPUT | wc -w" className="bg-black/40 border border-outline rounded px-3 py-2 text-xs text-white outline-none focus:border-blue-500 font-mono" />
                                                                <p className="text-[10px] text-foreground-subtle">Previous step output is available as $NODE_INPUT</p>
                                                            </div>
                                                        )}
                                                        {node.type === 'condition' && (
                                                            <>
                                                                <div className="flex flex-col gap-1">
                                                                    <label className="text-[10px] font-bold text-foreground-muted uppercase">Keyword Match</label>
                                                                    <input value={node.data.keyword || ''} onChange={(e) => updateNode(node.id, { keyword: e.target.value })}
                                                                        placeholder="error" className="bg-black/40 border border-outline rounded px-3 py-2 text-xs text-white outline-none focus:border-blue-500" />
                                                                </div>
                                                                <div className="grid grid-cols-2 gap-3">
                                                                    <div className="flex flex-col gap-1">
                                                                        <label className="text-[10px] font-bold text-foreground-muted uppercase">If Found</label>
                                                                        <input value={node.data.ifTrue || ''} onChange={(e) => updateNode(node.id, { ifTrue: e.target.value })}
                                                                            className="bg-black/40 border border-outline rounded px-3 py-2 text-xs text-white outline-none focus:border-blue-500" />
                                                                    </div>
                                                                    <div className="flex flex-col gap-1">
                                                                        <label className="text-[10px] font-bold text-foreground-muted uppercase">If Not Found</label>
                            ...
                            {/* Run Bar */}
                            <Card className="px-5 py-3 bg-black/30 border-outline-subtle flex items-center gap-4">
                                <input type="text" placeholder="Pipeline input..." value={pipelineInput} onChange={(e) => setPipelineInput(e.target.value)}
                                    disabled={executing} className="bg-black/40 border border-outline rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500/50 flex-1 disabled:opacity-50 disabled:cursor-not-allowed" />
                            ...
                            {/* Execution Results */}
                            {lastResult && (
                                <Card className="bg-black/20 border-outline-subtle p-4 space-y-3">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-[11px] font-bold text-foreground-muted uppercase tracking-wide flex items-center gap-2">
                                            <Clock size={12} />
                                        ...
                                        {lastResult.steps?.map((step, i) => (
                                            <div key={i} className="flex items-start gap-3 p-2 rounded-lg bg-black/20">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="text-xs text-foreground-secondary font-semibold">{step.node_name}</div>
                                                    <pre className="text-[10px] text-foreground-muted mt-0.5 whitespace-pre-wrap break-all max-h-24 overflow-y-auto font-mono">
                                                        {step.output}
                                                    </pre>
                                                </div>
                                            </div>
                        ...
                        <div className="flex-1 flex items-center justify-center">
                            <div className="text-center space-y-4 opacity-40">
                                <div className="w-20 h-20 bg-hover rounded-3xl flex items-center justify-center mx-auto border border-outline">
                                    <Play className="w-10 h-10 text-foreground-muted" />
                                </div>
                                <div>
                                    <h3 className="text-lg font-bold text-foreground-secondary">{t('pipelines.title')}</h3>
                                    <p className="text-sm text-foreground-muted">Select or create a pipeline to get started.</p>
                                    <p className="text-xs text-foreground-subtle mt-1">Chain LLM inference, shell commands, and filters into automated sequences.</p>
                                </div>
                            </div>
                        </div>
                    )}
