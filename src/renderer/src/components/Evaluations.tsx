    ...
    return (
        <div className="h-full flex flex-col text-white overflow-hidden pb-4">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col gap-6">
                ...
                {/* Active Model Banner */}
                <div className="bg-black/20 border border-outline rounded-xl p-4 flex items-center justify-between">
                    <div>
                        <h3 className="text-sm font-semibold text-foreground-muted uppercase tracking-wide mb-1">{t('evaluations.model')}</h3>
                        {activeModel ? (
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-green-500" />
                                <span className="text-lg font-bold">{cleanModelName(activeModel.name)}</span>
                                <span className="text-xs text-foreground-muted font-mono ml-2">({activeModel.id.split('/').pop()})</span>
                            </div>
                        ) : (
                    ...
                    <div className="flex items-center gap-2">
                        <label className="text-xs text-foreground-muted uppercase tracking-wide">Questions</label>
                        <select
                            title="Number of questions per benchmark"
                            value={sampleCount}
                            onChange={e => setSampleCount(Number(e.target.value))}
                            disabled={runningEval !== null}
                            className="bg-hover text-foreground-secondary border border-outline text-xs rounded px-2 py-1 outline-none disabled:opacity-50"
                        >
                                    ...
                                    {benchmarks.map(b => (
                                        <tr key={b.id} className="hover:bg-hover transition-colors">
                                            <td className="px-5 py-4">
                                                <div className="font-semibold text-foreground-secondary">{b.name}</div>
                                                <div className="text-xs text-foreground-muted mt-1">
                                                    <span className="bg-hover px-1.5 py-0.5 rounded border border-outline">{sampleCount} of 30 questions</span>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4 text-foreground-muted">{b.type}</td>
                                            <td className="px-5 py-4 text-right">
                                                {runningEval === b.id ? (
                                                    <div className="flex flex-col items-end gap-1.5">
                                                        ...
                                                        <div className="w-24 h-1.5 bg-black/50 rounded-full overflow-hidden border border-outline">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {history.map((h, i) => (
                                        <tr key={i} className="hover:bg-hover transition-colors">
                                            <td className="px-5 py-4">
                                                <div className="font-semibold text-foreground-secondary">{h.model}</div>
                                                <div className="text-xs text-foreground-muted mt-1">{h.date}</div>
                                            </td>
                                            <td className="px-5 py-4">
                                                <span className="bg-blue-500/10 text-blue-300 border border-blue-500/20 px-2 py-0.5 rounded text-xs">{h.bench}</span>
                                            </td>
                                            <td className="px-5 py-4 text-right">
                                                <div className="text-lg font-bold font-mono text-white">{h.score.toFixed(1)}<span className="text-xs text-foreground-muted ml-1">%</span></div>
                                                {h.total && <div className="text-[10px] text-foreground-subtle">{h.total} questions</div>}
                                            </td>
                                        </tr>
                                    ))}
                                    {history.length === 0 && (
