import { Clock, Cpu, Hash, Activity } from 'lucide-react'
import type { TelemetryData } from './types'

interface TelemetrySidebarProps {
  telemetry: TelemetryData
  isOpen: boolean
}

const STATE_COLORS: Record<string, string> = {
  thinking: 'bg-green-400',
  tool_calling: 'bg-yellow-400',
  waiting_human_approval: 'bg-blue-400 animate-pulse',
  done: 'bg-gray-500',
  error: 'bg-red-500',
}

function formatMs(ms: number): string {
  if (ms < 1000) return `${Math.round(ms)}ms`
  const s = ms / 1000
  if (s < 60) return `${s.toFixed(1)}s`
  return `${Math.floor(s / 60)}m ${Math.round(s % 60)}s`
}

export function TelemetrySidebar({ telemetry, isOpen }: TelemetrySidebarProps) {
  if (!isOpen) return null

  const dotColor = STATE_COLORS[telemetry.state] || 'bg-gray-500'

  return (
    <div className="w-64 border-l border-outline-subtle bg-hover flex flex-col overflow-hidden shrink-0">
      {/* Header */}
      <div className="px-3 py-2 border-b border-outline-subtle">
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${dotColor}`} />
          <span className="text-xs font-medium text-white capitalize">{telemetry.agent || 'agent'}</span>
          <span className="text-[10px] text-foreground-muted ml-auto">{telemetry.state.replace(/_/g, ' ')}</span>
        </div>
      </div>

      {/* Stats */}
      <div className="px-3 py-3 space-y-2 border-b border-outline-subtle">
        <div className="flex items-center gap-2 text-xs text-foreground-muted">
          <Cpu size={12} className="shrink-0 text-foreground-muted" />
          <span>Tokens</span>
          <span className="ml-auto text-white font-mono">{telemetry.tokensUsed.toLocaleString()}</span>
        </div>
        {telemetry.tokenBudget > 0 && (
          <div className="space-y-1">
            <div className="flex items-center justify-between text-[10px] text-foreground-muted">
              <span>Budget</span>
              <span>{Math.round(telemetry.budgetFraction * 100)}%</span>
            </div>
            <div className="h-1 bg-hover rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-300 ${
                  telemetry.budgetFraction > 0.9 ? 'bg-red-500' :
                  telemetry.budgetFraction > 0.7 ? 'bg-yellow-500' :
                  'bg-blue-500'
                }`}
                style={{ width: `${Math.min(100, telemetry.budgetFraction * 100)}%` }}
              />
            </div>
          </div>
        )}
        <div className="flex items-center gap-2 text-xs text-foreground-muted">
          <Clock size={12} className="shrink-0 text-foreground-muted" />
          <span>Elapsed</span>
          <span className="ml-auto text-white font-mono">{formatMs(telemetry.elapsedMs)}</span>
        </div>
        <div className="flex items-center gap-2 text-xs text-foreground-muted">
          <Hash size={12} className="shrink-0 text-foreground-muted" />
          <span>Iteration</span>
          <span className="ml-auto text-white font-mono">{telemetry.iteration}</span>
        </div>
      </div>

      {/* Action timeline */}
      <div className="flex-1 overflow-y-auto px-3 py-2">
        <div className="flex items-center gap-1.5 mb-2">
          <Activity size={12} className="text-foreground-muted" />
          <span className="text-[10px] font-bold tracking-wide text-foreground-muted uppercase">Timeline</span>
        </div>
        {telemetry.actions.length === 0 ? (
          <p className="text-[11px] text-foreground-subtle">No actions yet</p>
        ) : (
          <div className="space-y-1">
            {telemetry.actions.map((a, i) => (
              <div key={i} className="flex items-start gap-2 text-[11px]">
                <div className="w-1 h-1 rounded-full bg-blue-400 mt-1.5 shrink-0" />
                <div className="min-w-0">
                  <span className="text-foreground-secondary font-medium">{a.action}</span>
                  {a.detail && <span className="text-foreground-muted ml-1 truncate">{a.detail}</span>}
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}
