...
const STATUS_BADGE: Record<string, { label: string; color: string }> = {
    ...
    rejected: { label: 'Skipped', color: 'text-foreground-muted bg-hover' },
...
function StepRow({ step, index }: { step: PlanStep; index: number }) {
    ...
    const color = ACTION_COLOR[step.action] || 'text-foreground-muted'
    ...
    return (
        <div className={`flex items-start gap-2.5 px-3 py-2 rounded-lg transition-colors ${
            isRunning ? 'bg-blue-500/5 border border-blue-500/10' : 'bg-hover'
        }`}>
            <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                <span className="text-[10px] text-foreground-subtle font-mono w-4 text-right">{index + 1}</span>
                {isRunning ? (
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <span className="text-[11px] font-mono text-foreground-secondary truncate">{step.file}</span>
                    <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${statusInfo.color}`}>
                        {statusInfo.label}
                    </span>
                </div>
                <p className="text-[10px] text-foreground-muted mt-0.5 leading-relaxed">{step.description}</p>
            </div>
        </div>
    )
}
...
export function PlanCard({ meta, onApprove, onReject }: PlanCardProps) {
    ...
    return (
        <div className={`rounded-xl border overflow-hidden transition-colors ${
            ...
            isRejected ? 'border-outline bg-hover' :
            'border-outline bg-hover'
        }`}>
            {/* Header */}
            <button
                type="button"
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    {isRejected && (
                        <XCircle size={12} className="text-foreground-muted" />
                    )}
                </div>
            ...
            {isPending && (
                <div className="flex items-center gap-2 px-4 py-2.5 border-t border-outline-subtle bg-black/20">
                    <button
                        type="button"
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-hover hover:bg-active text-foreground-muted text-xs font-medium rounded-lg transition-colors"
                    >
                        <X size={11} />
                        Reject
                    </button>
